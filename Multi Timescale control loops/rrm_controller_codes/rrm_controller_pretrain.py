...
COOL_OFF_PERIOD = 60 * 60        # 1 hour cool-off after rollback
...
MIN_CHANGE_INTERVAL_DEPLOY = 4 * 60 * 60  # 4 hours (3-day run)
...
# Deploy (days 0-3)
SLOW_LOOP_PERIOD_DEPLOY = 10 * 60      # 10 minutes
MIN_CHANGE_INTERVAL_DEPLOY = 4 * 60 * 60  # 4 hours


# or 5
ROLLBACK_DELTA = 0.15
COOL_OFF_PERIOD = 60 * 60        # 1 hour
...
def visualize_rrm_graph_full(snapshot: dict,
    ...
    if not ap_channels:
        ap_channels = [0]
    num_colors = max(len(ap_channels), 1)
...
def compute_guardrail_kpis(snapshot: Dict[str, dict],
    ...
    """
    Compute KPI-style metrics for guardrails:

      - median throughput (index) for edge clients
      - P95 latency (if available)
      - P95 retries (network-wide)
      - airtime_eff_index: proxy for RU/utilization from PHY rates + retries
      - roaming steer success, P50 roam time (from fast loop / logs)
      - churn_rate (same as reward)
    """
    edge_throughputs = []      # effective throughput for edge clients (0..1)
    all_latencies = []         # ms
    all_retries = []           # counts
    airtime_eff_indices = []   # per-AP proxy for airtime efficiency

    # QoE-related
    for ap_id, t in snapshot.items():
        clients = t.get("clients", [])
        bw_mhz = float(t.get("bandwidth", 20)) or 20.0  # avoid div by zero

        # per-AP effective throughput (dimensionless 0..num_clients)
        thr_eff_clients = []
        ...
        for c in clients:
            ...
            # ----- effective throughput index for this client -----
            ...
            thr_eff = thr_norm * (1.0 - retries_norm)   # 0..1
        ...
        # ----- airtime efficiency index per AP -----
        # Sum of effective throughput over clients, normalized by bandwidth:
        #   - More "work" done with same or less bandwidth => better efficiency.
        if thr_eff_clients:
            total_eff_thr = sum(thr_eff_clients)        # 0..num_clients
            # Scale to 20 MHz baseline so 40 MHz doesn't automatically look better
            airtime_index = total_eff_thr * (20.0 / bw_mhz)
            airtime_eff_indices.append(airtime_index)

    med_edge_thr = float(np.median(edge_throughputs)) if edge_throughputs else 0.0
    ...
    # Roaming KPIs from fast loop / orchestration (you feed this in)
    steer_success = None
    p50_roam = None
    if fast_loop_stats:
        ...
        steer_success = steers_ok / max(1, total_steers)
        p50_roam = float(np.median(roam_times)) if roam_times else None

    # churn = fraction of APs changed this step
    churn_rate = len(changed_aps) / max(1, len(snapshot))

    return {
        "median_edge_thr": med_edge_thr,        # 0..1
        "p95_latency_ms": p95_latency,          # ms
        "p95_retries": p95_retries,            # count
        "airtime_eff_index": mean_airtime_eff,  # proxy for RU/util
...
def on_telemetry_message(client, userdata, msg):
    try:
        ...
        # ---- MINIMAL CLI PRINT ----
        print(f"[TELEMETRY] received from AP '{ap_id}'")
...
def can_apply_site_change(now: float) -> bool:
    window_start = now - 24 * 60 * 60  # last 24h
    recent = [ts for ts, _ in SITE_CHANGE_LOG if ts >= window_start]
    return len(recent) < SITE_CHANGE_BUDGET_PER_DAY
...
def _log(event: dict, path: str = CONTROLLER_LOG_PATH):
    ...
    Each line is a JSON object like:
      {
        "ts":  ...,
        "step": ... or null,
        "phase": "pretrain" | "deploy" | null,
        "event": "string_event_type",
        "data": { ... any additional fields ... }
      }
    """
    rec = {
        "ts": time.time(),
    ...
    try:
        ...
        # Don't crash the controller for logging issues
...
def visualize_ap_channel_coloring(snapshot: dict,
    ...
    if unique_channels:
        cmap = plt.colormaps.get_cmap("tab10", len(unique_channels))
        channel_to_color = {ch: cmap(i) for i, ch in enumerate(unique_channels)}
    else:
        cmap = plt.colormaps.get_cmap("tab10", 1)
        channel_to_color = {}
...
def slow_loop_controller(q_net: GNNQNetwork,
    ...
    while True:
        ...
        if prev_snapshot is not None:
            changed_aps = detect_config_changes(prev_snapshot, snapshot)
            reward = compute_step_reward(snapshot, changed_aps)
            kpis = compute_guardrail_kpis(snapshot, changed_aps, fast_loop_stats=None)

            if step_idx < BASELINE_WARMUP_STEPS:
                baseline_rewards.append(reward)
                update_kpi_windows(kpis, is_baseline=True)
            else:
                rl_rewards.append(reward)
                update_kpi_windows(kpis, is_baseline=False)

            if step_idx == BASELINE_WARMUP_STEPS:
                baseline_configs = snapshot_configs(prev_snapshot)
                rl_enabled = True
                _log({
                    "step": step_idx,
                    "phase": phase,
                    "event": "rl_enabled_after_warmup",
                    "data": {
                        "baseline_ma": float(np.mean(baseline_rewards)) if baseline_rewards else None,
                    },
                })
                print("[SAFE] Baseline warmup finished; RL enabled.")

            violated, reason = False, "ok"
            
            if rl_enabled and step_idx >= BASELINE_WARMUP_STEPS and len(baseline_rewards) >= 3 and len(rl_rewards) >= 3:
                baseline_ma = float(np.mean(baseline_rewards))
                rl_ma = float(np.mean(rl_rewards))
                underperforming = rl_ma < baseline_ma - ROLLBACK_DELTA
                violated, reason = check_guardrails(now)
                if violated:
                    _log({
                        "step": step_idx,
                        "phase": phase,
                        "event": "kpi_guardrail_violation",
                        "data": {
                            "reason": reason,
                ...
                if training_mode:
                    if violated:
                        ...
                        print(f"[SAFE][PRETRAIN] Violation #{guardrail_violations}: {reason}")
                        reward -= GUARDRAIL_NEG_REWARD
                        _log({
                            "step": step_idx,
                            "phase": phase,
                            "event": "pretrain_guardrail_neg_reward",
                            "data": {
                                "reason": reason,
                    ...
                    if violated:
                        guardrail_violations += 1
                        print(f"[SAFE] Violation #{guardrail_violations}: {reason}")
                        _log({
                            "step": step_idx,
                            "phase": phase,
                            "event": "deploy_guardrail_violation",
                            "data": {
                                "reason": reason,
                                "guardrail_violations": guardrail_violations,
                                "underperforming": underperforming,
                    ...
                    if underperforming or guardrail_violations >= MAX_GUARDRAIL_VIOLATIONS:
                        _log({
                            "step": step_idx,
                            "phase": phase,
                            "event": "rollback_and_cooloff",
                            "data": {
                                "underperforming": underperforming,
                                "guardrail_violations": guardrail_violations,
                                "cooldown_until": cooldown_until + COOL_OFF_PERIOD,  # will be overridden below
                            },
                        })
                        print(f"[SAFE] ROLLBACK & COOL-OFF initiated.")
                        apply_configs_via_mqtt(baseline_configs)
                        cooldown_until = now + COOL_OFF_PERIOD
                        rl_rewards.clear()
                        for k in kpi_rl: kpi_rl[k].clear()
                    else:
                        guardrail_violations = 0
            ...
            if training_mode:
                log_experience(prev_snapshot, prev_actions, reward, snapshot, step_idx,
                               violated_guardrail=violated, guardrail_reason=reason,
                               step_explanations=prev_step_explanations)

        # ========= 2. GRAPH + CHANNEL PLAN (CRITICAL CHANGES HERE) =========
        
        # A. Build G_rl (SPARSE) for GNN & Visualization
        # Uses overlap_factor. If channels differ (e.g. 1 & 6), edge is REMOVED.
        G_rl = build_networkx_interference_graph(snapshot)
        
        # # B. Visualize the Real Interference State
        # try:
        #     visualize_interference_graph(G_rl, step_idx)
        # except Exception as e:
        #     print(f"[VIS] Error: {e}")
        ...
        # Build CONFLICT Graph: edges based on physical RSSI; overlap is implicitly 1 here
        G_conflict = build_conflict_graph(snapshot, rssi_threshold=-85.0)
        
        
        # --- DEBUG: print conflict graph structure ---
        print("[CONFLICT] nodes:", list(G_conflict.nodes()))
        print("[CONFLICT] edges:")
        for u, v, d in G_conflict.edges(data=True):
            print(f"   {u} -- {v}, rssi={d.get('rssi', 0):.1f}, weight={d.get('weight', 0):.2f}")
        # --------------------------------------------
        
        # Seed plan for stickiness:
        # - On first run, use whatever channels the APs are currently on.
        # - Later, reuse the previous plan as the "current" plan.
        if prev_channel_plan is None:
            seed_plan = {
                ap_id: t.get("channel")
                for ap_id, t in snapshot.items()
        ...
        if need_new_plan:
            if G_conflict.number_of_edges() == 0:
                # No conflicts: keep whatever channels we already have
                channel_plan = seed_plan
                print(f"[DSATUR] No conflicts, keeping current channels: {channel_plan}")
            else:
                # Run Sticky DSATUR on Conflict Graph
                channel_plan = min_overlap_dsatur_sticky(
                    G_conflict,
                    CANDIDATE_CHANNELS,
                    current_plan=seed_plan,
            ...
            channel_plan = prev_channel_plan
            print(f"[DSATUR] Reusing previous plan.")
            
        # # Optional: visualize the conflict graph with channels
        # try:
        #     visualize_channel_graph(G_conflict, channel_plan, step_idx)
        # except Exception as e:
        #     print(f"[VIS] Channel graph error: {e}")
        ...
        # Applies Channel Plan + RL Actions
        ...
        print(f"[SLOW LOOP] step={step_idx} took {elapsed:.1f}s, sleeping {sleep_time:.1f}s\n")
...
def main():
    ...
    if args.phase == PHASE_DEPLOY:
        # load trained model from previous day (D-1, D0, D1...)
        if os.path.exists("cql_rrm_gnn_current.pth"):
            q_net.load_state_dict(torch.load("cql_rrm_gnn_current.pth", map_location="cpu"))
            print("[MAIN] Loaded trained weights from cql_rrm_gnn_current.pth")
        else:
            print("[MAIN][WARN] cql_rrm_gnn_current.pth not found, running untrained policy.")
