#!/usr/bin/env python3
# Robust VecResize block (full content) — paste whole file and save as Test_epy_block_5.py
import numpy as np
...
class blk(gr.sync_block):
    ...
    def work(self, input_items, output_items):
        ...
        if (self._calls % 1000) == 0:
            try:
                print(f"[VecResize] work called: n_avail={n_avail}, n_space={n_space}, calls={self._calls}", flush=True)
            except Exception:
                pass
        for i in range(m):
            vec = inbuf[i]
            ...
            if arr.size >= IMG_IN:
                if arr.size != IMG_IN:
                    ...
                    if now - self._last_warn > 5.0:
                        try:
                            print(f"[VecResize] warning: incoming length {arr.size} (expected {IMG_IN}) — using first frame", flush=True)
                        except Exception:
                            pass
                        self._last_warn = now
                frame = arr[:IMG_IN]
